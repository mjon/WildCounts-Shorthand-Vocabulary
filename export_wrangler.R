...
cat ("Within each category are **types**, **values**, and **syntax**. **Types** are the different concepts, like \"taxon name\", \"age\", and \"initial position compass bearing\". Within each type are one or more **values**. For example, the type \"age\" has the values \"adult\", \"juvenile\", and \"baby\". Within each value is then one, or sometimes several, shorthand syntax options for entering that value. For example, the shorthand syntax for \"adult\" is `a`. \n\n", file = textfile, append = TRUE)
